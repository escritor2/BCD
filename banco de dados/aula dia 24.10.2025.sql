...
FROM cliente INNER JOIN venda
 ON cliente.codcli = venda.codcli
GROUP BY cliente.nome, venda.duplic;
...
insert into ex values ("maria eduarda");
insert into ex values ("laury");
insert into fx values ("fernada ");
insert into fx values ("nico ");
insert into fx values ("douglas");
insert into fx values ("carla");
...
from cliente inner join venda
on cliente.codcli = venda.codcli
where year(venda.vencto) = 2004 or month(venda.vencto) = 11
...
where cliente.nome ="%pctec%";

SELECT C.nome, V.vencto       
FROM cliente C INNER JOIN venda V ON C.codcli = V.codcli
WHERE V.vencto BETWEEN '2004-11-01' AND '2004-11-30'
ORDER BY V.vencto;

SELECT C.nome, V.duplic, V.vencto
FROM cliente C INNER JOIN venda V ON C.codcli = V.codcli
